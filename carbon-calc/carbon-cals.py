...
# Tailpipe factors via DEFRA / EPA / ARAI.
# Electric factor represents a fixed-scope benchmark estimate of location-based
# average grid electricity generation intensity (Scope 2 indirect) sourced from
# Central Electricity Authority (CEA) India / UK DEFRA Open Government Licence.
EMISSION_FACTORS = {
...
def analyze_traffic_and_efficiency(
    ...
    if fuel_lower == "electric":
        ...
        # and high aerodynamic resistance at sustained highway speeds.
        if avg_speed_kmh <= 25:
        ...
        # Diesel engines have lower idle consumption and higher thermal efficiency in traffic.
        if avg_speed_kmh <= 25:
        ...
        # Standard gasoline light-duty curve (DOE / ORNL empirical studies)
        if avg_speed_kmh <= 25:
...
def compare_routes(
    ...
    # Unrounded floats prevent premature precision loss
    normal_co2_raw = normal.get("_raw_co2_kg", normal["co2_kg"])
    rz_co2_raw = route_zero.get("_raw_co2_kg", route_zero["co2_kg"])

    # Signed differences: positive = savings/improvement, negative = deterioration/worse route
...
if __name__ == "__main__":
    result = compare_routes(
        ...
        vehicle_class="sedan", fuel_type="petrol"
    )
